...
def train_dann(model, tokenizer, source_loader, target_loader, num_epochs=3, lr=2e-5, alpha=1.0, device="cuda", class_weights=None):
    """Huấn luyện DANN với Source và Target loader riêng biệt"""
    model = model.to(device)
    optimizer = AdamW(model.parameters(), lr=lr)
    ...
    for epoch in range(1, num_epochs + 1):
        model.train()
        # Dùng zip để lặp qua cả 2 loader (lấy loader ngắn nhất làm mốc)
