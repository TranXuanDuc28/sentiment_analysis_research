...
def generate_demo_plot(results_dir="results", output_path="results/plots/research_story.png"):
    ...
    # Dữ liệu demo cho 12 kịch bản hoàn chỉnh
    demo_data = {
    ...
    for group_name, sids in groups.items():
        for sid in sids:
            plot_data.append({"Chặng": group_name, "Kịch bản": sid, "F1-Macro": demo_data[sid]})
    
    df = pd.DataFrame(plot_data)
    plt.figure(figsize=(13, 7))
    sns.set_style("whitegrid")
    ax = sns.barplot(x="Chặng", y="F1-Macro", hue="Kịch bản", data=df, palette="viridis")
    plt.title("Tiến trình Hiệu năng Nghiên cứu Toàn diện", fontsize=16, fontweight='bold', pad=20)
    plt.ylim(0, 1.0)
